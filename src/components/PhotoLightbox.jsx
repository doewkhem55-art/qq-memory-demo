...
export default function PhotoLightbox({
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-center justify-center bg-black/88 px-4 py-5 text-white backdrop-blur-xl sm:px-8"
      role="dialog"
      aria-modal="true"
      aria-label="照片大图预览"
      onMouseDown={(event) => {
    ...
    >
      <button
        type="button"
        onClick={onClose}
        className="focus-ring absolute right-4 top-4 z-10 flex h-11 w-11 items-center justify-center rounded-full border border-white/12 bg-white/[0.08] text-white shadow-xl shadow-black/30 transition hover:bg-white/[0.16]"
        aria-label="关闭大图预览"
      >
        <X size={19} />
      </button>
      ...
      {hasMultiple ? (
        <>
          <button
            type="button"
            onClick={goPrevious}
            className="focus-ring absolute left-3 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/12 bg-white/[0.08] text-white shadow-xl shadow-black/30 transition hover:bg-white/[0.16] sm:left-5"
            aria-label="上一张照片"
          >
            <ChevronLeft size={22} />
          </button>
          <button
            type="button"
            onClick={goNext}
            className="focus-ring absolute right-3 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/12 bg-white/[0.08] text-white shadow-xl shadow-black/30 transition hover:bg-white/[0.16] sm:right-5"
            aria-label="下一张照片"
          >
      ...
      <div className="flex h-full w-full max-w-6xl flex-col gap-4">
        <div className="flex min-h-0 flex-1 items-center justify-center">
          {showImage ? (
            <img
              ...
              className="max-h-full max-w-full rounded-[1.75rem] object-contain shadow-2xl shadow-black/50"
              onError={() => {
        ...
        <div className="memory-float-caption mx-auto w-full max-w-4xl rounded-[1.35rem] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-2 text-xs text-sky-100">
              <span className="memory-float-caption rounded-full px-3 py-1">
